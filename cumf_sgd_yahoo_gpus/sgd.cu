...
void *read_problem_thread(void *argument)
{
    ...
    for(long long idx = 0;idx < arg->size;idx ++)
    {
        ...
        //if(idx > 0 && idx%1000000 == 0)printf("tid:%2d, progress: %%%.3f\n",arg->thread_id, 100.0*idx/arg->size);
    }
    fclose(fptr);
...
mf_model*sgd_train(mf_problem*tr, mf_problem*te, Parameter para)
{
    ...
    //collect the factor. scaling is used to make sure every rating is arount 1.
    SGDRate ave;
    SGDRate std_dev;
    SGDRate scale = 1.0;
