        ...
        for(int update_ite = 0; update_ite < update_count_this_block; update_ite ++)
        {
            ...
            for(int i = 0;i < update_vector_size;i++)
            {
                ...
                if(threadIdx.x == 0)
                {
                    ...
                    int global_update_id = ite*u_grid*v_grid*gridDim.x*update_count_per_block +
                                           (u_id*v_grid + v_id)*gridDim.x*update_count_per_block +
                                           blockIdx.x*update_count_per_block + 
                                           update_ite;
                                           
                    gpu_iter_err[global_update_id] = block_err;
                }
            #endif
...
__global__ void sgd_k128_kernel_hogwild_warp32_lrate_128threads(
                            const mf_node *R,
                            long long nnz,
                            half *p,
                            half *q,
