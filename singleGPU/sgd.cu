...
void scale_problem(mf_problem *prob, float scale, long long u_seg,
                   long long v_seg) {
...
mf_problem read_problem(string path) {
  ...
  for (int i = 0; i < 80; i++) {
    ...
    if (access(tmp_name.c_str(), F_OK) != -1)
  ...
  if (num_files <= 0) {
    ...
    while (true) {
      ...
      R[idx].rate = r;
      idx++;
      // if(idx > 0 && idx%100000000 == 0)printf("progress:
      // %%%.3f\n",100.0*idx/prob.nnz);
    }
    prob.R = R;
    ...
    for (int i = 0; i < num_files; i++) {
      ...
      pthread_create(&(threads[i]), NULL, read_problem_thread,
                     (void *)(&(pthread_arg_list[i])));
    }
    ...
    prob.n = -1;
    for (int i = 0; i < num_files; i++) {
      if (pthread_arg_list[i].max_m >= prob.m)
        prob.m = pthread_arg_list[i].max_m;
      if (pthread_arg_list[i].max_n >= prob.n)
        prob.n = pthread_arg_list[i].max_n;
    }
...
void grid_problem(mf_problem *prob) {
  ...
  if (prob->ux == 1)
    ...
    u_seg = (long long)ceil((double)prob->m / prob->ux);
  if (prob->vy == 1)
    v_seg = prob->n;
  else
    v_seg = (long long)ceil((double)prob->n / prob->vy);
...
__global__ void random_init(curandState *state, int state_size, half *array,
...
void scale_model(mf_model *model, float scale) {
  ...
  for (long long i = 0; i < ((long long)model->m) * model->k; i++)
    model->floatp[i] = model->floatp[i] * factor_scale;

  for (long long i = 0; i < model->n * model->k; i++)
    model->floatq[i] = model->floatq[i] * factor_scale;
...
void shuffle_model(mf_model *model, int *inv_p_map, int *inv_q_map) {
  auto inv_shuffle1 = [](float *vec, int *map, int size, int k) {
    for (int pivot = 0; pivot < size;) {
      ...
      for (SGDIndex d = 0; d < k; d++)
        swap(*(vec + (long long)pivot * k + d),
             *(vec + (long long)next * k + d));
